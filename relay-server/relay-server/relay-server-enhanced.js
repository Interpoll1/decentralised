...
server.on('request', async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/api/posts') {
    ...
    try {
      const rows = await queryMySQL(
        `SELECT soul, data FROM gun_nodes WHERE soul REGEXP '^(v3/communities|v2/communities|communities)/[^/]+/posts/post-[^/]+$' ORDER BY JSON_EXTRACT(data, '$.createdAt') DESC LIMIT ${limit}`, []
      );
      const posts = [];
      const seen = new Set();
      for (const row of rows || []) {
