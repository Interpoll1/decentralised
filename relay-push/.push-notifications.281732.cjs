...
const { attachEmailFallback } = require('./email-notifications');
 ...
 * @param {{
 *   serviceAccountPath?: string, tokenStorePath?: string,
...
function attachPush(app, opts = {}) {
  ...
  // Email fallback for the reach FCM does not have: the web build, a device
  // that never registered, or a token FCM has since dropped. Silently absent
  // unless RESEND_API_KEY is set. Pass `email: false` to opt out entirely.
  let email = null;
  if (opts.email !== false) {
    try {
      email = attachEmailFallback(app, {
        ...(opts.email || {}),
        json: express_json,
        guard,
        authenticate,
      ...
      console.warn('[Email] Fallback not attached:', err.message);
    }
  }
  ...
  async function notifyChatMessage({ toUserId, fromUserId, senderName, force = false }) {
    ...
    const tokens = sender ? store.tokensFor(toUserId) : [];
    if (!tokens.length) {
      // Nothing to push to — this is exactly what the email fallback is for.
      if (email) await email.notifyChatMessage({ toUserId });
      return;
    }
    ...
    // Every token was dead or errored, so the user was not actually reached.
    if (!delivered && email) await email.notifyChatMessage({ toUserId });
  }

  return { notifyChatMessage, store, email };
}
