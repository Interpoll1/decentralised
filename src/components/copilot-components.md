...
|---|---|---|
| `VoteForm.vue` | Full voting form for a poll. Handles option selection, duplicate-vote checking, private-poll invite-code reservation/finalization, calls `AuditService.authorizeVote()` before voting, then emits the receipt immediately after the chain vote lands. The backend authorization step now returns a short-lived reservation token, and confirm sends that token so only the authorizing session can commit. Backend confirm, Gun count sync, and invite-code finalization continue in the background so slow relays do not leave the form spinning or falsely report the whole vote as failed. Watches `poll.id` so reused `/vote/:pollId` instances refresh state instead of keeping stale "already voted" state. | `poll`, `inviteCode?`, `requiresInviteCode?`, emits `vote-submitted` |
