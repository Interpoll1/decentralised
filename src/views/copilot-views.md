...
| `CommunityPage.vue` | Community detail page with feed, metadata, and join/share actions. In the mixed `all` filter, it now promotes one poll near the top when polls are present but would otherwise be buried below many posts, so polls are visible without deep scrolling. On direct route reloads it now performs a short `communityStore.loadCommunities()` bootstrap before selecting the community to reduce cold-start misses, and emits detailed `[CommunityDebug]` loading logs when `localStorage.interpoll_community_debug === 'true'`. For encrypted communities, creator/admin devices now see a **Device Access Management** panel that loads pending requests and approved key-ring entries from relay backend endpoints, with approve and remove+rotate actions wired to the new key-management flow. |
| `PollDetailPage.vue` | Full poll page with inline vote submission, results gating, duplicate-vote checks, and receipt-first submission flow. The backend authorization step now returns a short-lived reservation token that the page passes to backend confirm after the chain vote, so only the same authorizing session can commit. After `chainStore.addVote()` succeeds it marks the vote locally, routes to the receipt immediately, and lets backend confirm plus Gun/MySQL follow-up sync continue in the background so slow or outdated relays do not bounce the user back into a second submission attempt. Invite and standalone vote links now preserve `communityId` in the query string so community-scoped fallback loading still works on direct opens and refreshes. |
